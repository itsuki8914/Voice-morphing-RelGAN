    ...
    while iteration <= num_iterations:
        ...
        if iteration % 1000 ==0 or iteration==10:
            for q in range(2):
                ...
                for file in glob.glob(eval_A_dir + '/*.wav'):
                    ...
                    print(np.min(f0_converted)," ",np.max(f0_converted))
                    if(np.min(f0_converted)<0 or np.max(f0_converted)>1000):
                        print(log_f0s_means[x_atr])
                        print(log_f0s_stds[x_atr])
                        print(log_f0s_means[y_atr])
                        print(log_f0s_stds[y_atr])
                        print(log_f0s_mean_AB)
                        print(log_f0s_std_AB)
        ...
        if iteration % 1000 ==0 or iteration==10:
            ...
            for file in glob.glob(eval_A_dir + '/*.wav'):
                ...
                wav = wav_padding(wav=wav, sr=sampling_rate, frame_period=frame_period, multiple=4)
                f0, timeaxis, sp, ap = world_decompose(wav=wav, fs=sampling_rate, frame_period=frame_period)
                log_f0s_mean_AB = (1-alpha)*log_f0s_means[x_atr]+alpha*log_f0s_means[y_atr]
                log_f0s_std_AB = (1-alpha)*log_f0s_stds[x_atr]+alpha*log_f0s_stds[y_atr]
                f0_converted = pitch_conversion(f0=f0, mean_log_src=log_f0s_means[x_atr], std_log_src=log_f0s_stds[x_atr],
                                                mean_log_target=log_f0s_mean_AB, std_log_target=log_f0s_std_AB)
                coded_sp = world_encode_spectral_envelop(sp=sp, fs=sampling_rate, dim=num_mcep)
                coded_sp_transposed = coded_sp.T
